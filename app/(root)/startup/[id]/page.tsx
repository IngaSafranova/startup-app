...
const Page = async ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  // Pararel data fetching. Takes as long as longest one fetch to complete
  const [post, {select: editorPosts}] = await Promise.all([
    client.fetch(STARTUP_BY_ID_QUERY, { id }),
    client.fetch(PLAYLIST_BY_SLUG_QUERY,{slug:'editor-picks'})
  ])

  // Sequencial data fetching, when next fetch is done after first is complete.
  // it takes sum of time of all fetches
  ...
  return (
    <>
      ...
      <section className='section_container' >
        <img src={post.image} alt="tumbnail" className='w-full h-auto rounded-xl' />
